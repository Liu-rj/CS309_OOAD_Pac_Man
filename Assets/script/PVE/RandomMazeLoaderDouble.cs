...
public class RandomMazeLoaderDouble : MonoBehaviour
{
    ...
    private int pacman_pos_in_maze2 = 0;
    private Vector3 pacman_pos2 = new Vector3(0, 0, 0);
    ...
    void Start()
    {
        ...
        _players = new[] {blinky, clyde, inky, pinky, pacman, pacman2};
    ...
    private void PauseGame()
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            {
                _players[i].GetComponent<Player1Move>().enabled = false;
            }
            else
            {
                _players[i].GetComponent<Player2Move>().enabled = false;
            }
        }
    }
    ...
    public void ClickOnBack()
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            {
                _players[i].GetComponent<Player1Move>().enabled = true;
            }
            else
            {
                _players[i].GetComponent<Player2Move>().enabled = true;
            }
        }
    }
    ...
    private void DrawMaze()
    {
        for (int i = 0; i < _maze.GetLength(0); i++)
        {
            for (int j = 0; j < _maze.GetLength(1); j++)
            {
                ...
                else if (int.Parse(_maze[i, j]) > 0 && int.Parse(_maze[i, j]) < 10)
                {
                    ...
                    {
                        pacman_pos_in_maze2 = int.Parse(_maze[i, j]) - 4;
                        pacman_pos2 = new Vector3(j, 0.5f, -i);
                    }
                }
            }
        }
    ...
    {
        _players[pacman_pos_in_maze].GetComponent<Player1Move>().Init(pacman_pos, 28, 31);
        _players[pacman_pos_in_maze2].GetComponent<Player2Move>().Init(pacman_pos2, 28, 31);
    }
}
